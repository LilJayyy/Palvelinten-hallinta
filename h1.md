...
Pythonin valituksen poistaminen käyttökokemuksen parantamiseksi onnistui niin, että syötin komennot yllä mainitut komennot eli **`micro hosts.ini`** ja **`cat hosts.ini`** ja sisällöksi tuli: 
**`localhost
[all:vars]
...
Vaihdoin tässä kohtaa huvikseen tekstin sisällön:
`- copy:
    dest: /tmp/hello-ansible
    content: "See you at Mordor!\n"`


`site.yml` -tiedosto oli sisällöltään
`- hosts: all
  roles:
    - hello`
...
Kello oli tässä kohtaa 0:16 joten olin ilahtunut, kun playbookin ajaminen sujui alla olevasti. Kuvaan on päässyt myös aiemman Cowsay-komentoriviohjelman asennuksen mukana tullut lehmäjengi.
...
Asennus ja määrittely tapahtui aika nopeasti ja seuraavaksi vielä testaus ajamalla`ansible-playbook site.yml`.
